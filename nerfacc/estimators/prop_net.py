...
class PropNetEstimator(AbstractEstimator):
    ...
    def sampling(
        ...
        self.distoration_cache.append(intervals)
    ...
    @torch.no_grad()
    def get_nerf_s_vals(self):
        if len(self.distoration_cache) == 0:
            return torch.zeros((), device=self.device)
        dt_loss = self.distoration_cache.pop().vals
        self.distoration_cache = []
        return dt_loss
